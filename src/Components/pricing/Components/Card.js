...
function Card({ imgSrc, title, list, price, plan, btnText }) {
    ...
    return (
        <div className='pricing-card my-3 px-1 lg:p-5'>
            <div className='bg-white shadow-xl p-8 rounded'>
                <div className='w-48 h-40 mx-auto'>
                    <img src={imgSrc} className='xl:ml-0 w-full h-full' alt={title} />
                </div>
                <div className='card-body'>
                    <h3 className='font-medium text-2xl text-center font-bold mt-4 mb-6'>{title}</h3>
                    <ul>
                        {
                            list.map((item, index) => {
                    ...
                    <h3 className='text-indigo-800 text-4xl font-bold text-center'>${price}</h3>
                    <span className='text-sm block text-center text-gray-500 italic font-semibold'>
                        {plan}
                    </span>
                    <a href='#' className='font-medium text-white font-bold bg-orange-500 w-3/4 block text-center mx-auto p-3 mt-5 rounded transition ease-in-out duration-200 hover:bg-orange-600'>
                        {btnText}
                    </a>
                </div>
            </div>
